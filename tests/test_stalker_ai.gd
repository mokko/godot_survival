extends SceneTree
## Headless check: the Dusk Stalker is aimed. It adopts a territory where it
## stands (so baked stalkers hunt instead of marching to world origin), patrols
## between two land legs, chases a player inside its territory, damages them on
## contact with a cooldown, and drops the chase when the player leaves.
...
func _init() -> void:
	...
	# 3. Chase: player inside territory and inside sight radius.
	fake.position = stalker.global_position + Vector3(4.0, 0.0, 0.0)
	await _wait(0.3)
	...
	# 4. Contact damage: exactly HIT_COOLDOWN apart, DAMAGE each time.
	await _wait(2.0)
	if fake.hits < 1:
	...
	# 6. A reconfigured stalker honours what it is given (spawner path).

...
## main menu offers a Quit button wired to the quit handler.
...
func _init() -> void:
	...
	# Quit must exist and be wired; we don't press it (that would quit the
	# test process before it can report).
	var quit_btn: Button = splash.get_node_or_null("Center/VBox/Quit")
	var quit_ok: bool = quit_btn != null and quit_btn.text == "Quit" \
			and not quit_btn.disabled
	if quit_ok:
		var wired := false
		for c in quit_btn.pressed.get_connections():
			if (c["callable"] as Callable).get_method() == "_on_quit_pressed":
				wired = true
		quit_ok = wired

...
## Run: cd project && snap run godot-4 --headless --script res://build_terrain.gd
...
func _build_terrain(island: GDScript) -> void:
	...
	if err != OK:
		...
		quit(1)
		return
	err = ResourceSaver.save(packed, "res://scenes/terrain.tscn")
...
func _build_water() -> void:
	...
	if err != OK:
		...
		quit(1)
		return
	err = ResourceSaver.save(packed, "res://scenes/water.tscn")
...
func island_caldera_x() -> float:
	return load("res://island.gd").CALDERA_CENTER.x


func island_caldera_z() -> float:
	return load("res://island.gd").CALDERA_CENTER.y


func island_lake_level() -> float:
	return load("res://island.gd").LAKE_LEVEL
